...
int main(int argc, char *argv[]) {
    ...
    mem_limit.rlim_cur = 50 * 1024 * 1024;  // 50 MB soft limit
    mem_limit.rlim_max = 100 * 1024 * 1024;  // 100 MB hard limit
  ...
  for (;;) {
      pid = fork();
        if (pid == 0) {
            ...
            if (setrlimit(RLIMIT_AS, &mem_limit) == -1 || setrlimit(RLIMIT_CPU, &cpu_limit) == -1) {
                perror("setrlimit");
                exit(1);
            }
  for (;;) {
