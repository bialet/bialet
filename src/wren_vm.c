...
static void callForeign(WrenVM* vm, ObjFiber* fiber, WrenForeignMethodFn foreign,
                        int numArgs) {
...
static Value validateSuperclass(WrenVM* vm, Value name, Value superclassValue,
  ...
  if(!IS_CLASS(superclassValue)) {
    return wrenStringFormat(vm, "Class '@' cannot inherit from a non-class object.",
                            name);
  }
  ...
  if(numFields == -1 && superclass->numFields > 0) {
    return wrenStringFormat(
        vm, "Foreign class '@' may not inherit from a class with fields.", name);
  }
...
static void bindForeignClass(WrenVM* vm, ObjClass* classObj, ObjModule* module) {
  ...
  if(methods.allocate == NULL && methods.finalize == NULL) {
    ...
    if(strcmp(module->name->value, "random") == 0) {
      methods =
          wrenRandomBindForeignClass(vm, module->name->value, classObj->name->value);
    }
#endif
  }
...
static Value resolveModule(WrenVM* vm, Value name) {
  ...
  if(resolved == NULL) {
    vm->fiber->error =
        wrenStringFormat(vm, "Could not resolve module '@' imported from '@'.", name,
                         OBJ_VAL(importer));
    return NULL_VAL;
  }
